...
def statistics(kernel, burger_count, runs):
    kernel.set_stochastic_mode(True)
    duration_samples = []
    for i in range(runs):
        kernel.scheduler_.time_ = 0.0
        kernel.total_cost_.duration_ = 0.0
        kernel.produce_item(ItemCount("FinishedHamburger", burger_count, ""))
        kernel.run()
...
def main():
    ...
    kernel.add_to_stock(ItemCount("CleaningAgent",  inf, "cardinal"))
